...
IMAGE_ADD="IMAGE_INSTALL:append = \"linux-firmware-rpidistro-bcm43430 mosquitto mosquitto-clients python3-paho-mqtt python3-rpi-bme280 python3-smbus2 python3-project-scripts nano v4l-utils python3 ntp wpa-supplicant libgpiod libgpiod-tools libgpiod-dev i2c-tools fbida fbgrab imagemagick\""
...
CORE_IM_ADD="CORE_IMAGE_EXTRA_INSTALL += \"bme-config camera-config\""
...
if [ $local_coreimadd1_info -ne 0 ];then
        echo "Append ${CORE_IM_ADD1} in the local.conf file"
        echo ${CORE_IM_ADD1} >> conf/local.conf
        
else
        echo "${CORE_IM_ADD1} already exists in the local.conf file"
fi
######################################################################################
# Add ssh support
IMAGE_F="IMAGE_FEATURES += \"ssh-server-openssh\""
